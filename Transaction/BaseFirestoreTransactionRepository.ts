  ...
  implements ITransactionRepository<T>
{
  ...
  async execute(queries: IFireOrmQueryLine[]): Promise<T[]> {
    ...
    return this.extractTFromColSnap(d, this.transaction, this.tranRefStorage);

    // return this.transaction
    //   .get(query)
    //   .then((c) =>
    //     this.extractTFromColSnap(c, this.transaction, this.tranRefStorage)
    //   );
  }
  ...
  findById(id: string) {
    ...
    return this.transaction
      ...
      .then((c) =>
        c.exists
          ? this.extractTFromDocSnap(c, this.transaction, this.tranRefStorage)
          : null
      );
  }
