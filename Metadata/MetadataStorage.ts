...
export interface SubCollectionMetadataWithSegments
  extends SubCollectionMetadata {
  segments: string[];
}
...
export class MetadataStorage {
  ...
  public getCollection = (pathOrConstructor: string | IEntityConstructor) => {
    ...
    if (typeof pathOrConstructor === "string") {
      ...
      collection = this.collections.find((c) =>
        arraysAreEqual(c.segments, collectionSegments)
      );
    } else {
  ...
  public setCollection = (col: CollectionMetadata) => {
    ...
    if (existing) {
      throw new Error(
        `Collection with name ${existing.name} has already been registered`
      );
    }
    ...
    while (colsToUpdate.length) {
      ...
      const parent = this.collections.find(
        (p) => p.entityConstructor === c.parentEntityConstructor
      );
      c.segments = parent?.segments.concat(c.name) || [];
      getWhereImParent(c.entityConstructor).forEach((col) =>
