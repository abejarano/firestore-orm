...
export interface ISingleBatchRepository<T extends IEntity>
  extends IBatchRepository<T> {
  commit(): Promise<unknown>;
}

export interface IFirestoreBatchSingleRepository<T extends IEntity>
  extends IBatchRepository<T> {
  commit(): Promise<void>;
}
...
export interface IBaseRepository<T extends IEntity> {
  ...
  // update(
  //   item: PartialWithRequiredBy<T, "id">
  // ): Promise<PartialWithRequiredBy<T, "id">>;
...
export type ISubCollection<T extends IEntity> = IRepository<T> & {
  ...
  runTransaction<R = void>(
    executor: (tran: ITransactionRepository<T>) => Promise<R>
  ): Promise<R>;
};
...
export type EntityConstructorOrPathConstructor<T extends IEntity> = {
  new (): T;
};
