  ...
  implements IRepository<T>
{
  ...
  protected initializeSubCollections = (
    ...
    } = require("../Transaction/FirestoreTransaction");
    ...
    this.colMetadata.subCollections.forEach((subCol) => {
      ...
      if (tran && tranRefStorage) {
        const firestoreTransaction = new FirestoreTransaction(
          tran,
          tranRefStorage
        );
  ...
  protected initializeSerializedObjects(entity: T) {
    Object.keys(entity).forEach((propertyKey) => {
      if (
        Reflect.getMetadata(serializeKey, entity, propertyKey) !== undefined
      ) {
        ...
        if (Array.isArray(subData)) {
          (entity as unknown as { [key: string]: unknown })[propertyKey] =
            subData.map((value) => {
  ...
  protected extractTFromColSnap = (
    ...
    return q.docs
      .filter((d) => d.exists)
      .map((d) => this.extractTFromDocSnap(d, tran, tranRefStorage));
  };
