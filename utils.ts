...
import { SubCollectionMetadata } from "./Metadata/MetadataStorage";
  ...
  Object.entries(serializableObj).forEach(([propertyKey, propertyValue]) => {
    if (Reflect.getMetadata(serializeKey, obj, propertyKey) !== undefined) {
      if (Array.isArray(propertyValue)) {
        (serializableObj as { [key: string]: unknown })[propertyKey] =
          propertyValue.map((element) => serializeEntity(element, []));
      } else {
        (serializableObj as { [key: string]: unknown })[propertyKey] =
          serializeEntity(propertyValue as Partial<T>, []);
      }
    }
  });
